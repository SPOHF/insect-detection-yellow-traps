...
def _write_sample(base: Path, split: str, name: str, suffix: str = ".jpg") -> None:
    (base / "images" / split).mkdir(parents=True, exist_ok=True)
    (base / "labels" / split).mkdir(parents=True, exist_ok=True)
    (base / "images" / split / f"{name}{suffix}").write_bytes(b"x")
...
def test_load_train_samples_accepts_jpeg_images(tmp_path: Path) -> None:
    _write_sample(tmp_path, "train", "trapA__001", suffix=".jpeg")
    ...
    samples, _ = load_train_samples(data_yaml, group_delimiter="__")
    assert samples[0].image.suffix == ".jpeg"
