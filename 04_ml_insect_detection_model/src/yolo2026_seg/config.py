...
class TrainConfig:
    ...
    # Output
    export_best_to: Path = Path("04_ml_insect_detection_model/weights/yolo2026_seg_best.pt")
...
def load_train_config(path: str | Path) -> TrainConfig:
    ...
    cfg = TrainConfig(
        ...
        model=str(raw.get("model_architecture", "yolo26m-seg.pt")),
        project=Path(raw.get("project", "runs/train")),
        name=str(raw.get("experiment_name", "yolo2026_seg")),
        ...
        export_best_to=Path((raw.get("outputs") or {}).get("best_model_path", "04_ml_insect_detection_model/weights/yolo2026_seg_best.pt")),
    )
...
def load_infer_config(path: str | Path) -> InferConfig:
    ...
    return InferConfig(
        ...
        save_vis=bool(outputs.get("save_visualizations", True)),
    )
