...
def _scatter_svg(title: str, points: list[dict]) -> str:
    ...
    points_svg = ''.join(
        f'<circle cx="{x_for(float(p["temp_avg"])):.2f}" cy="{y_for(float(p["avg_population"])):.2f}" r="4" fill="#0f766e">'
    ...
    return (
        ...
        f'<text x="14" y="{margin_top + inner_h / 2:.2f}" text-anchor="middle" transform="rotate(-90, 14, {margin_top + inner_h / 2:.2f})" fill="#334155" font-size="12">Avg insects per upload</text>'
        '</svg></div>'
    )
...
def _render_exploratory_report_html(question: str, answer: str, context: dict) -> str:
    ...
    graph_checks = [
        f"Population chart: {'ok' if len(joined_rows) >= 2 else 'insufficient data'}",
        f"Temperature chart: {'ok' if len(scatter_rows) >= 2 else 'insufficient matched weather data'}",
        f"Population vs temperature scatter: {'ok' if len(scatter_rows) >= 2 else 'insufficient matched weather data'}",
    ]

    table_rows = ''.join(
        '<tr>'
        f'<td>{escape(str(row["week_start"]))}</td>'
        f'<td>{row["temp_avg"]:.3f}' + '</td>'
...
<head>
  ...
  <style>
    ...
    .chart-card {{ background: #fff; border: 1px solid #cbd5e1; border-radius: 10px; padding: 12px; }}
    .chart-svg {{ width: 100%; height: auto; background: #f8fafc; border-radius: 8px; }}
...
<body>
  <div class="card">
    ...
    <div class="meta">
      ...
      <p><strong>Scope:</strong> {escape(str(context.get('scope', '-')))}</p>
  ...
  <div class="charts">
    {_line_chart_svg('Average Insect Population by Week', [row['week_start'][5:] for row in joined_rows], [float(row['avg_population']) for row in joined_rows], 'Avg insects per upload', '#0f766e')}
    {_line_chart_svg('Average Temperature by Week', [row['week_start'][5:] for row in scatter_rows], [float(row['temp_avg']) for row in scatter_rows], 'Temperature (C)', '#2563eb')}
    {_scatter_svg('Insect Population vs Temperature (Weekly)', scatter_rows)}
  </div>
  <div class="card">
